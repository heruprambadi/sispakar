...
class CMS_Model extends CI_Model
{
    ...
    {
        $query = $this->db->query("SELECT user_id, user_name, real_name, email FROM ".cms_table_name('user')." WHERE
    ...
    public function cms_do_register($user_name, $email, $real_name, $password)
    {
        ...
        $this->db->insert(cms_table_name('user'), $data);
    ...
    public function cms_do_change_profile($user_name, $email, $real_name, $password = NULL)
    {
        ...
        $this->db->update(cms_table_name('user'), $data, $where);
    }
    ...
    public function cms_generate_activation_code($identity, $send_mail = FALSE, $reason = 'FORGOT')
    {
        ...
        // generate query
        $query = $this->db->query("SELECT user_name, real_name, user_id, email FROM ".cms_table_name('user')." WHERE
                    (user_name = '" . addslashes($identity) . "' OR email = '" . addslashes($identity) . "') AND
                    $where_active");
        if ($query->num_rows() > 0) {
            $row              = $query->row();
    ...
    {
        $query = $this->db->query("SELECT user_id FROM ".cms_table_name('user')." WHERE
                    (activation_code = '" . md5($activation_code) . "')");
        if ($query->num_rows() > 0) {
            $row     = $query->row();
            $user_id = $row->user_id;
    ...
    {
        $query = $this->db->query("SELECT activation_code FROM ".cms_table_name('user')." WHERE
    ...
    {
        $SQL      = "SELECT user_name FROM ".cms_table_name('user')." WHERE user_name='" . addslashes($username) . "'";
        $query    = $this->db->query($SQL);
    ...
    public function cms_third_party_login($provider)
    {
        ...
        $user_id = $this->cms_user_id();
        $user_id    = !isset($user_id)||is_null($user_id)?0:$user_id;
        $query   = $this->db->select('user_id')->from(cms_table_name('user'))->where('auth_' . $provider, $identifier)->get();
        if ($query->num_rows() > 0) { // get user_id based on auth field
            ...
            if ($user_id == FALSE) {
                $query = $this->db->select('user_id')->from(cms_table_name('user'))->where('email', $third_party_email)->get();
                if ($query->num_rows() > 0) {
                    $row     = $query->row();
                    $user_id = $row->user_id;
                }
            ...
            if ($user_id != FALSE) {
                ...
                $duplicate = TRUE;
                while ($duplicate) {
                    $query = $this->db->select('user_name')->from(cms_table_name('user'))->where('user_name', $new_user_name)->get();
                    if ($query->num_rows() > 0) {
                        $query         = $this->db->select('user_name')->from(cms_table_name('user'))->get();
        ...
        $query = $this->db->select('user_id, user_name, email, real_name')->from(cms_table_name('user'))->where('user_id', $user_id)->get();
        if ($query->num_rows() > 0) {
            $row = $query->row();
            $this->cms_user_id($row->user_id);
            $this->cms_user_name($row->user_name);
